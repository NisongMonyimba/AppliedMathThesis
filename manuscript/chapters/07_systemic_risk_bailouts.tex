...
$c_2$ & 1.8 & Terminal default penalty \\
...
under the corrected $B_t$ equation of Appendix~A. \textbf{Correction note:} an
earlier version of this example used $c_2 = 5.0$, which lies above the stable
fixed point $A^\ast = \kappa\lambda + \sqrt{(\kappa\lambda)^2 + c_1\lambda} = 2.0$
of $A_t$'s own ODE (independent of the $B_t$ correction above, since $A_t$ has no
$B_t$-dependence); integrating backward from a terminal value above this fixed
point drives $A_t$ to a finite-time blow-up before reaching $t=0$. The baseline
parameter $c_2$ has been corrected to $1.8$
(\Cref{tab:sr-baseline-parameters}), which lies inside the stable band
$(-1.0, 2.0)$ and keeps $A_t, \Gamma_t, \Pi_t$ positive and bounded across the
full horizon; this was verified with two independent solvers (LSODA and Radau)
agreeing to four significant figures. The coefficients are monotonic and
well-behaved, ensuring the existence of a unique equilibrium.
...
counter-cyclical relationship with slope $-\Pi_t/\gamma \approx -1.07$ (recomputed
under the corrected $B_t$ equation and the corrected $c_2 = 1.8$; see the
