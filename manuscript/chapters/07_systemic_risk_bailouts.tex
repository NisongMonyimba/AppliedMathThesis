...
foundational model of Carmona, Fouque, and Sun (2015) by incorporating a benevolent
...
and lending was introduced by Carmona, Fouque, and Sun (2015). Cuchiero, Reisinger,
and Rigger (2024) analysed optimal bailout strategies via mean-field control; Ren and
Firoozi (2024) studied risk-sensitive MFGs with common noise. \textbf{The present
...
contribution relative to Carmona, Fouque, and Sun (2015) is the inclusion of an
active government bailout policy; relative to Cuchiero, Reisinger, and Rigger (2024),
...
aggregate shocks. This setup follows the framework of Carmona, Fouque, and Sun
(2015), who first proposed a mean-field game model of systemic risk with inter-bank
borrowing and lending. Subsequent extensions include Garnier, Papanicolaou, and Yang
(2013), who used large deviations theory to analyse systemic risk in a mean-field
model, and Firoozi et al.\ (2019), who incorporated common noise and Markovian
latent processes. More recently, Ren and Firoozi (2024) studied risk-sensitive MFGs
with common noise and applied them to interbank markets, while Cuchiero, Reisinger,
and Rigger (2024) analysed optimal bailout strategies using mean-field control
techniques.
...
\begin{itemize}
    ...
    the banking system. Following Carmona, Fouque, and Sun (2015), we use a
    ...
    LQ-MFG models (see, e.g., Cuchiero, Reisinger, and Rigger, 2024).
...
version of the indicator-based default cost used in Carmona, Fouque, and Sun (2015).
...
This assumption is consistent with the low-default regime analysed in Garnier,
Papanicolaou, and Yang (2013) using large deviations theory. Under this
...
volatility. The large deviations analysis of Garnier, Papanicolaou, and Yang (2013)
...
\begin{align}
    ...
    \dot{F}_t &= -\frac{D_t^2 \sigma^2}{2\lambda} - \frac{\sigma_0^2}{2}(A_t + C_t)
        - D_t b_t, & F_T &= 0. \notag
\end{align}
These equations are derived in Appendix~A. The linear terms capture the effect of
...
with common noise (Carmona and Delarue, 2018, Vol.\ I, Theorem 2.12). The key
...
equilibrium reduces to the laissez-faire MFG of Carmona, Fouque, and Sun (2015).
...
hazard. From the Riccati system, we can compute $\eta_\alpha$ explicitly; typical
values in our numerical experiments range from $-0.3$ to $-0.6$, indicating a
substantial but not complete crowding-out of private effort.
...
This finding aligns with the results of Ren and Firoozi (2024), who showed that
...
Riccati coefficients $A_t, \Gamma_t$, and $\Pi_t$ over the time horizon. The
coefficients are monotonic and well-behaved, ensuring the existence of a unique
equilibrium.
...
counter-cyclical relationship with slope $-\Pi_t/\gamma \approx -6.24$. As the
