...
weighted Wasserstein-2 metric, via synchronous coupling and a Gronwall argument. The
contraction constant is given explicitly in terms of the drift Lipschitz constant, the
Frobenius norm of the common-noise coefficient, and bounds on the relevant Lions
derivatives. Existence and uniqueness of the mean-field equilibrium follow from the
Banach fixed-point theorem, together with the associated $\varepsilon$-Nash property
for the finite-player game. Relative to the closely related quantitative results of
\citet{jacksontangpi2024,jacksonmeszaros2025}, our target is the fixed-point map
constructing the equilibrium itself -- with a contraction constant intended as a
convergence-rate and stopping criterion for numerical solvers -- rather than the
$N$-player convergence rate to an assumed equilibrium. We illustrate the theory,
including a direct numerical verification of the underlying Riccati system via two
...
\begin{assumption}[Smallness condition]
\label{ass:smallness}
The control-sensitivity constant $C_\alpha$ of \eqref{eq:control-lipschitz} and the
BSDE-stability constant $C_{\mathrm{BSDE}}$ of \eqref{eq:bsde-estimate} -- both
determined by \Cref{ass:lipschitz-drift,ass:concavity,ass:regularity} -- satisfy
$3\,C_{\mathrm{BSDE}}\,C_\alpha^2 < 1$.
\end{assumption}

\begin{remark}[On \Cref{ass:smallness}]
\label{rem:smallness-early}
This condition is used only in \Cref{app:algebra} to close the Gronwall argument
underlying \Cref{thm:stability}; it is stated here, ahead of its use, so that the
full list of hypotheses required for the main theorem is visible in one place. It
is a genuine restriction on the model (not merely a proof artefact): it bounds how
sensitive the optimal control and adjoint process may be to perturbations in the
state and measure, relative to each other. It is always satisfiable by working on a
sufficiently short horizon or under a sufficiently strong concavity constant
$\lambda_H$, and is verified explicitly for the linear-quadratic benchmark of
\Cref{sec:example}. \citet{jacksontangpi2024} show that a structurally similar
restriction can be replaced by a displacement-monotonicity condition in a related
(though distinct) setting; adapting that route to relax
\Cref{ass:smallness} here is identified as future work in \Cref{sec:intro}.
\end{remark}
...
\label{thm:stability}
Under Assumptions~\ref{ass:lipschitz-drift}--\ref{ass:smallness}, for all
...
Under Assumptions~\ref{ass:lipschitz-drift}--\ref{ass:smallness}, for any
...
exactly as in the derivation of \eqref{eq:constants}. Existence and uniqueness of the
fixed point then follow from
...
of the FBSDE system established above. The detailed calculations underlying the
stability estimate \eqref{eq:stability-estimate} and the contraction bound
\eqref{eq:contraction} are deferred to Appendices~\ref{app:algebra}
and~\ref{app:contraction-proof}, respectively.
\end{proof}
...
$[0,T]$ and yields an explicit, finite contraction constant $\rho < 1$ in
...
\paragraph{What this enables computationally.} The contraction bound
\eqref{eq:contraction} justifies a simple fixed-point (Picard) iteration as a
provably convergent algorithm for computing the mean-field equilibrium numerically,
with the number of iterations needed for a target tolerance $\varepsilon$
bounded explicitly by $\lceil \log(\varepsilon/\|\mu^{(0)}-\mu^*\|_{\lambda,T}) /
\log\rho\rceil$. This is used as a design criterion in the companion numerical
paper referenced below.

\paragraph{Limitations.} The present analysis has three main limitations, each
discussed at the point where it arises: the contraction constant depends on
regularity constants $C_{\mathrm{reg}}$ that are not fully explicit outside the
linear-quadratic case (\Cref{sec:discussion}); closing the Gronwall argument
rigorously requires the smallness condition \eqref{eq:smallness-condition}, whose
relaxation via displacement monotonicity (following the strategy of
\citealp{jacksontangpi2024}) we identify as a concrete direction for future work
(\Cref{sec:intro}); and the stability estimate requires absolute continuity and
...
\paragraph{Step 3: \Cref{ass:smallness} closes the estimate.} The term
...
the left. This is precisely where \Cref{ass:smallness}, stated in
\Cref{sec:setup} ahead of its use,
\begin{equation}
    3\,C_{\mathrm{BSDE}}\,C_\alpha^2 < 1,
    \label{eq:smallness-condition}
\end{equation}
is invoked: it allows the self-referential term to be absorbed. Under
...
\begin{remark}[Verifying \Cref{ass:smallness} in practice]
As noted in \Cref{rem:smallness-early}, this is a standing structural assumption
on the model, checkable directly once $C_\alpha, C_{\mathrm{BSDE}}$ are computed
from the model primitives; in the linear-quadratic case of \Cref{sec:example} it
can be verified directly from the explicit Riccati coefficients.
\end{remark}
