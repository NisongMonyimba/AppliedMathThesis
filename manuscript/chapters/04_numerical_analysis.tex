...
studied by \citep{chassagneuxcrisandelarue2019}, who established convergence of a
particle method for a class of mean-field FBSDEs without common noise. The CEMRACS
2017 project (see \citep{angiulietal2024} investigated numerical probabilistic methods
...
analysis builds on the work of \citep{achdouporretta2016} for deterministic MFG PDEs
and on \citep{lordpowellshardlow2014} for SPDEs. The weak error analysis for
particle discretisations of McKean--Vlasov SDEs follows the framework of \citep{bencheikh2020}. The BSDE regression method is based on the foundational work of \citep{gobetlemorwarin2005} and \citep{bouchardtouzi2004}.
...
We recall the FBSDE system \eqref{eq:fbsde-system} that characterises the MFG
equilibrium. Let $(\mu^*, \alpha^*, X^*, p, q, r)$ denote the unique solution whose
existence is guaranteed by Theorem~\ref{thm:wellposedness}. For numerical
...
nonparametric regression on a set of basis functions. Following \citep{gobetlemorwarin2005}, we project the random variable inside the expectation onto a
...
terminal condition and the driver are sufficiently smooth (see \citep{bouchardtouzi2004}, or \citep{gobetlemorwarin2005}. The proof uses the fact that the BSDE can be
...
mean-square stability of the SPDE discretisation (see \citep{lordpowellshardlow2014}, Chapter 10), we obtain the condition
\[
...
for a similar SPDE is given in \citep{lordpowellshardlow2014}, Theorem 10.7). The
...
discretisation error term. Specifically, we show in Lemma~C.9 of Appendix~A that
\[
...
\section{Numerical Experiments: Linear-Quadratic Benchmark}
...
\begin{itemize}
    ...
    stepping with step sizes $\Delta t \in \{2^{-5}, 2^{-6}, 2^{-7}, 2^{-8}\}$.
    \item \textbf{Finite-difference method}: spatial domain $[-3,3]$ with mesh sizes
    $\Delta x \in \{0.1, 0.05, 0.025, 0.0125\}$, time step chosen to satisfy the CFL
    condition \eqref{eq:cfl-condition}.
...
\begin{table}[ht]
\centering
\caption{Convergence results for the LQ-MFG benchmark}
...
requiring minimal tuning and scaling well with the number of agents. For
higher-dimensional problems ($d \geq 3$), the particle method suffers from the curse
of dimensionality in the Wasserstein convergence rate $O(M^{-1/d})$, and alternative
approaches such as tensor networks or deep learning-based density estimation may be
necessary. These extensions are discussed as open problems in Chapter 10.
...
\begin{tabular}{lccc}
\toprule
\textbf{Scheme} & \textbf{Time (seconds)} & \textbf{Memory (MB)} & \textbf{Scaling
    with $d$} \\
\midrule
...
algorithms developed in Chapter 5 and the empirical finance application in Chapter 9.
The ability to accurately simulate the equilibrium measure flow is essential for
generating synthetic data, validating learning algorithms, and ultimately extracting
tradable signals from real market data.

\begin{remark}[Reproducibility note --- relation to repository code]
The numerical experiments of this section are reproduced in
\texttt{simulations/euler\_milstein/run\_convergence\_simulation.py} in the
accompanying code repository, using common random numbers (CRN) to isolate
discretisation error from Monte Carlo noise. \textbf{That implementation measures
\emph{strong} pathwise error (order 1.0, since $\sigma, \sigma_0$ are additive/constant
in this model, so the Milstein correction vanishes and Euler--Maruyama attains its
maximal strong order), which is a different quantity from the \emph{weak}
(Wasserstein/measure-level) error reported in Table~\ref{tab:convergence-results}
above (order $\approx 0.48$--$0.49$). Both are correct simultaneously; they are not
in conflict, but care should be taken when citing a single "convergence rate" for this
chapter without specifying which error notion is meant.}
\end{remark}
