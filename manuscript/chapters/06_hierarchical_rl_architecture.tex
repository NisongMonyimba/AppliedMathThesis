...
MFGs originates from Carmona, Delarue, and Lachapelle (2014). The use of
multi-timescale RL for MFGs has been explored by Angiuli, Fouque, Lauri\`ere, and
Zhang (2024) for finite spaces, and by Zhou, Zhou, and Hu (2025) for continuous-time
...
(Carmona, Delarue, and Lachapelle, 2014)
\[
...
We select Soft Actor-Critic (SAC) (Haarnoja et al., 2018) as the fast critic. SAC is
...
We select Proximal Policy Optimization (PPO) (Schulman et al., 2017) as the
...
\subsection{Slow Meta-Learner: Twin Delayed DDPG (TD3) for Common-Noise Adaptation}
...
We select Twin Delayed Deep Deterministic Policy Gradient (TD3) (Fujimoto et al.,
2018) as the slow meta-learner. TD3 is an off-policy algorithm designed for
...
(Sutton and Barto, 2018), this fixed point is unique and stable.
...
\begin{itemize}
    ...
    \mu_{\mathrm{init}})$;
    \item Optionally, the first $K$ moments or the output of a set encoder (Zaheer
    et al., 2017).
\end{itemize}
