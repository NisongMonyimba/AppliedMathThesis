...
The script \texttt{simulations/euler\_milstein/run\_convergence\_simulation.py}
...
\paragraph{Sensitivity to mesh size.}
The finite-difference component with spatial refinement
$\Delta x \in \{0.1, 0.05, 0.025, 0.0125\}$ (CFL-coupled $\Delta t$) exhibits
rates of 0.95--0.99 in $\Delta x$, consistent with the predicted first-order
spatial convergence. No degradation is observed as $\Delta x \to 0$ within the
tested range.

\paragraph{Runtime scaling with particle count.}
Table~\ref{tab:runtime-scaling} reports wall-clock time for the Euler--Maruyama
particle system ($d=1$, $T=1$, $\Delta t = 2^{-6}$) as a function of $N$,
confirming the $O(N)$ complexity of the LQ mean-field-factored implementation.

\begin{table}[ht]
\centering
\caption{Runtime scaling with particle count (LQ mean-field factored, $d=1$)}
...
\noindent The $O(N)$ scaling is confirmed empirically. The per-particle time is
nearly constant ($\approx 42\,\mu$s) across three orders of magnitude, indicating
no superlinear overhead from the mean-field interaction (consistent with the
$O(N)$ factored algorithm of Chapter 6).

\paragraph{Comparison with baseline.}
As a baseline, we compare the equilibrium-feedback Euler--Maruyama scheme
(using the corrected Riccati $A_t, B_t$) against a na\"ive scheme that ignores
the mean-field coupling ($B_t \equiv 0$, i.e., each agent evolves as if it were
alone). The equilibrium scheme achieves strong error $O(\Delta t^{1.10})$; the
na\"ive scheme's error does not decrease with $\Delta t$ because it is dominated
by the mean-field modelling error. This confirms that the Riccati correction is
quantitatively significant, not merely a theoretical issue.
