...
% Same fix as the CV: load helvet for proper scalable Helvetica
% instead of falling back to ugly bitmap fonts.
\renewcommand{\familydefault}{\sfdefault}
\pagestyle{empty}

% \paragraph normally produces a small numbered sub-subsection
% heading. Here we redefine it as a "run-in" bold label that sits
% INLINE at the start of the paragraph (like "How I arrived here.")
% rather than on its own line -- this is the same visual style used
% in the draft text, and it's a common, clean look for statement-
% style documents (research statements, SOPs, cover letters).
\titleformat{\paragraph}[runin]{\bfseries}{}{}{}[.]
\titlespacing{\paragraph}{0pt}{8pt}{6pt}

% \parskip adds a blank gap between paragraphs (so we don't need
% blank lines to separate them visually). \parindent set to 0pt
% means paragraphs are NOT indented on the first line -- a common
% modern/clean style choice, also matching the CV's style.
...
\paragraph{Fit} [2--3 sentences to be tailored per program: name specific faculty whose work on
stochastic control, mean-field games, numerical SPDEs, or mathematical finance most closely
overlaps with this research program, and state concretely what about their recent work I would
want to build on or learn from.]
