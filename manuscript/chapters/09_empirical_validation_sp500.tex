...
\textbf{Critical data disclaimer, stated before any results below.} The
methodology in this chapter -- point-in-time CRSP/Compustat universe
construction, SEC Form 13F institutional holdings, CFTC COT futures
positioning, and the associated look-ahead-bias controls -- describes the
data pipeline that a genuine empirical study of this kind would require. The
accompanying simulation code does not have access to licensed CRSP,
Compustat, or SEC 13F microdata, and \textbf{the specific numerical results
reported in this chapter (Sharpe ratios, drawdowns, the 1.91 figure below,
and all comparison-strategy statistics) were generated from a synthetic data
panel with an embedded predictive signal, not from real market data.} This
chapter should be read as a methodologically complete specification of how
the model would be validated empirically, together with a demonstration
that the methodology produces internally consistent results on synthetic
data with known structure -- not as evidence that the strategy has been
validated on real markets. This distinction is stated explicitly in the
supplementary code's documentation but was not previously carried into this
chapter's prose with comparable prominence; that gap is corrected here.
\end{minipage}}
\end{center}
...
1.91 (net of transaction costs) \textbf{on the synthetic data described above},
statistically significant at $p = 0.002$ via block
...
calibration windows, discount functional forms, and subperiods, \textbf{all evaluated
on the same synthetic panel; robustness to these choices has not been established
on real market data.}
...
\textbf{Note on what follows:} this section specifies the data sources and
construction methodology that a real implementation of this empirical study
would use. As stated in the disclaimer at the start of this chapter, the
actual results reported below were generated from synthetic data with an
embedded predictive signal, since the licensed real-data sources described
here (CRSP, Compustat, SEC 13F, CFTC COT) were not available in the
environment where this thesis was produced. The methodology below is
presented in full because it specifies exactly what a future real-data
implementation would need to replicate.
...
\centering
\footnotesize
\caption{Out-of-sample performance metrics, January 2005 -- December 2024. \textbf{All
figures, including the S\&P 500 and momentum/reversal benchmark columns, are computed
on the synthetic data panel described in the chapter-level disclaimer (\Cref{sec:emp-data}),
not on actual historical market data.}}
...
\textbf{The comparison below is qualitative only: the 1.91 figure is measured on the
synthetic data panel described in \Cref{sec:emp-data}, and is not directly comparable
to Sharpe ratios reported for real strategies in the cited literature, which are
measured on actual historical returns.} With that caveat, the magnitude is broadly in
the range reported for published equity strategies over
similar time periods. The na\"ive 13F mean-reversion benchmark achieves 0.72, and
the 13F residual momentum benchmark achieves 0.89, both on the same synthetic panel.
The incremental performance of
the MFG strategy on this synthetic panel may be attributed to its unique focus on distributional
disequilibrium, which is not captured by traditional price-based factors or simple
holdings-based signals -- a structural claim that would need to be re-tested on real
data to be confirmed empirically.
