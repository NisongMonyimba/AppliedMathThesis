...
which in turn affects the master equation through the It\^o correction arising from
the quadratic variation of the stochastic measure flow $\mu_t$ itself. \textbf{The
precise form of this correction term is technically delicate (it involves a
second-order Lions derivative coupled with the common-noise covariance) and is given
rigorously in \citep{cardaliaguetetal2019}; we do not reproduce the exact expression
here, since an imprecise restatement risks introducing an error that the rigor of the
rest of this thesis aims to avoid.} For the purposes of motivating the hierarchical
RL architecture below, what matters is the qualitative structure: this operator
\begin{equation}
    \mathcal{L}_{\mathrm{common}} U(t,\mu) \;=\; O\big(\|\sigma_0\|_F^2\big)
    \label{eq:operator-common}
\end{equation}
is a second-order functional of $U$ in the measure argument, scaling with
$\|\sigma_0\|_F^2$.
...
\]
\textbf{This decomposition is presented as a qualitative organisational device
motivating the timescale separation used for the hierarchical RL architecture below,
not as a fully rigorous re-derivation of the common-noise master equation; the latter
is available in \citep{cardaliaguetetal2019}.} The timescale separation among
$\mathcal{L}_{\mathrm{idio}}, \mathcal{L}_{\mathrm{mf}}$,
...
\addlinespace
$\mathcal{L}_{\mathrm{common}}$ & $O(\|\sigma_0\|_F^2)$, second-order in
    $\delta U/\delta\mu$; see \citep{cardaliaguetetal2019} for the exact form &
    Common-noise transport & Slow &
    TD3 (Meta-Learner) \\
\bottomrule
\end{tabular}
}
