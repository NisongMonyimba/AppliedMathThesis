...
\section{Closed-Form Solution of the LQ-MFG}
...
is provided in Appendix~A. \textbf{The equations below reflect the corrected
derivation from Appendix~A, which identified and fixed sign errors in the $A_t$
equation and missing mean-field coupling terms in $B_t$--$F_t$; see Appendix~A
for the correction history.} The key equations for the quadratic terms are:
\begin{align}
    \dot{A}_t &= 2\kappa A_t + \frac{A_t^2}{\lambda} - c_1, & A_T &= c_2, \notag \\
    \dot{B}_t &= \kappa B_t + \frac{2A_t B_t}{\lambda} + \frac{B_t^2}{\lambda}
        - \kappa A_t, & B_T &= 0,
        \label{eq:sr-riccati-quadratic} \\
    \dot{C}_t &= -2\kappa B_t + \frac{2A_t C_t}{\lambda} + \frac{B_t^2}{\lambda}
        + \frac{2B_t C_t}{\lambda}, & C_T &= 0. \notag
\end{align}
The Riccati equation for $A_t$ is a scalar quadratic ODE, which admits a unique
positive solution on $[0,T]$ (verified numerically with the corrected sign; the
sign error in earlier versions caused blowup for $c_2 = 5.0$, which is now stable).
The equations for $B_t$ and $C_t$ are linear given $A_t$ and can be solved
sequentially.

The linear terms $D_t, E_t, F_t$ satisfy the following ODEs, which depend on the
bailout policy $b_t$ and include the mean-field coupling terms from the
Lions-derivative in the master equation:
\begin{align}
    \dot{D}_t &= \kappa D_t + \frac{A_t D_t}{\lambda} + \frac{B_t D_t}{\lambda}
        - (A_t+B_t) b_t, & D_T &= 0,
        \notag \\
    \dot{E}_t &= -\kappa D_t + \frac{A_t E_t}{\lambda} + \frac{B_t D_t}{\lambda}
        + \frac{B_t E_t}{\lambda} + \frac{C_t D_t}{\lambda}
        - (B_t+C_t) b_t, & E_T
        &= 0, \label{eq:sr-riccati-linear} \\
    \dot{F}_t &= \frac{D_t^2}{2\lambda} + \frac{D_t E_t}{\lambda}
        - (D_t+E_t) b_t - \frac{\sigma^2+\sigma_0^2}{2} A_t
        - \sigma_0^2 B_t - \frac{\sigma_0^2}{2} C_t, & F_T &= 0. \notag
\end{align}
These equations are derived in full in Appendix~A, including the mean-field coupling
terms arising from the Lions derivative of the master equation with respect to $\mu$.
The linear terms capture the effect of
the bailout policy on the value function and are essential for determining the
...
\end{equation}
\textbf{Note on sign convention:} despite the government \emph{minimising} cost,
the quadratic term in $\Pi_t$ has a positive sign. This arises because the optimal
control $b^* = -(1/\gamma)\Pi_t\bar\mu_t$ is substituted back into the HJB: the
cost term $(\gamma/2)b^{*2} = \Pi_t^2/(2\gamma)\bar\mu_t^2$ contributes positively,
yielding $\dot\Pi_t = 2\Gamma_t\Pi_t + \Pi_t^2/\gamma - c_1$.
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{p{1.2cm}p{5.2cm}p{2.8cm}}
\toprule
...
Riccati coefficients $A_t, \Gamma_t$, and $\Pi_t$ over the time horizon, computed
under the fully corrected system of Appendix~A. \textbf{Correction note (this
revision):} a previous revision reported a finite-time blow-up of $A_t$ at
$c_2=5.0$ and worked around it by lowering $c_2$ to $1.8$. A full re-derivation
of the master equation (Appendix~A) subsequently found that this blow-up was a
symptom of a genuine sign error in the $\dot{A}_t$ equation itself, independent
of the $B_t$ sign correction noted there: the correct equation is
$\dot{A}_t=2\kappa A_t+A_t^2/\lambda-c_1$ (the $A_t^2/\lambda$ and $c_1$ terms
both have the opposite sign from the previously stated equation). Under the
corrected equation, $A_t$ is well-behaved across the entire originally
intended parameter range, and \textbf{the original $c_2=5.0$ has been
restored} (\Cref{tab:sr-baseline-parameters}). The same re-derivation found
that $B_t$, $C_t$, $D_t$, $E_t$, and $F_t$ all required an additional
mean-field (Lions-derivative) coupling term, omitted in earlier revisions;
the complete corrected system is given in Appendix~A and was verified there
by direct PDE-residual substitution and by two independent numerical solvers
(LSODA and Radau) agreeing to all displayed digits.

\begin{table}[ht]
\centering
\caption{Riccati Coefficients at Selected Times (recomputed under corrected system)}
\label{tab:sr-riccati-coefficients}
\begin{tabular}{lccc}
\toprule
\textbf{Time $t$} & $A_t$ & $\Gamma_t=(A_t+B_t)/\lambda$ & $\Pi_t$ \\
\midrule
0.0 & 1.09 & 1.28 & 0.60 \\
0.5 & 1.44 & 1.64 & 0.59 \\
1.0 & 5.00 & 5.00 & 5.00 \\
...
counter-cyclical relationship with slope $-\Pi_t/\gamma \approx -0.59$ at $t=0.5$
(computed under the fully corrected system above and the restored $c_2 = 5.0$; see the
correction note preceding \Cref{tab:sr-riccati-coefficients}). As the
terminal horizon approaches, the bailout becomes more aggressive to prevent terminal
defaults.

\Cref{tab:sr-wasserstein-error} shows how the Wasserstein
distance $W_2(\mu_t^N, \mu_t^*)$ evolves for $N = 50, 100, 200$. The error decays as $1/\sqrt{N}$,
