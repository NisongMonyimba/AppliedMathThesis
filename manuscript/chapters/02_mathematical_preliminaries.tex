...
\begin{enumerate}
    ...
    the Burkholder--Davis--Gundy inequality, martingale representation, and Girsanov's
    theorem.
...
treatment, we recommend \citep{villani2009} for optimal transport, \citep{carmonadelarue2018},
...
$\pi^* \in \Pi(\mu,\nu)$ achieving the infimum (\citep{villani2009}, Theorem 5.10). The
...
$c(x,y) = |x-y|^2$. See \citep{villani2009}, Ch.\ 9) for the complete argument.
\end{proof}
...
financial applications (Chapter 9), the empirical market measure $\mu_t$ is a sum of
Dirac masses, and the equilibrium measure $\mu_t^*$ is estimated from discrete data.
...
operations (\citep{cuturi2013}. In Chapter 4, this regularised distance will be used to
...
be any couplings. By the gluing lemma (\citep{villani2009}, Lemma 7.6), there exists
...
$D_\mu F(\mu)$ and $v$. For a comprehensive treatment, see \citep{carmonadelarue2018},
Vol.\ I, Chapter 5).

\subsection{Law of Large Numbers in Wasserstein Distance}

\begin{theorem}[LLN Rate in $W_2$, \citep{fournierguillin2015}]
...
\begin{enumerate}[label=(\roman*)]
    ...
    appropriate moment conditions. (See \citep{fournierguillin2015}, Theorem 1 and
    Corollary 2.)
\end{enumerate}
\end{theorem}
...
Wasserstein space: for a portfolio of $d=50$ assets, the empirical measure converges
...
Theorem 3.1 --- allows the RL architecture (Chapters 5--6) to circumvent this
...
\subsection{Wasserstein Barycentre}

\begin{definition}[Fr\'echet Mean / Barycentre]
Given measures $\mu_1, \dots, \mu_K \in \mathcal{P}_2(\mathbb{R}^d)$ with weights
$\lambda_k > 0$, $\sum_k \lambda_k = 1$, the Wasserstein barycentre is
\[
    \bar{\mu} = \argmin_{\nu \in \mathcal{P}_2} \sum_{k=1}^K \lambda_k W_2(\nu,\mu_k)^2.
\]
\end{definition}

The barycentre exists and is unique when at least one $\mu_k$ is absolutely continuous
(\citep{aguehcarlier2011}. It is computed by the free-support Sinkhorn barycentre
algorithm (\citep{cuturidoucet2014}. \textit{Note: this concept is included for
completeness but is not directly used in the subsequent chapters.}
...
\begin{remark}[Significance for Theorem 3.1]
The It\^o isometry is used in the proof of Theorem 3.1: after deriving the Wasserstein
...
\[
    ...
    dW_t^i + \sigma_0 \nabla\phi(X_t^i) \cdot dW_t^0 + \frac{\sigma^2}{2}
...
\[
    ...
    \phi) \right\rangle dt + \underbrace{\frac{\sigma}{N}\sum_i \nabla\phi(X_t^i)\,
...
introduce nonlinear feedback into the normalisation. See Chapter 4 of \citep{carmonadelarue2018} for a detailed discussion.
\end{remark}
...
\begin{remark}
This is used in Theorem 3.1: to bound the expected supremum
$\mathbb{E}[\sup_t W_2^2(\mu_t^1,\mu_t^2)]$ by $4\mathbb{E}[W_2^2(\mu_T^1,\mu_T^2)]$,
...
\label{thm:girsanov}
Let $\theta : [0,T] \times \Omega \to \mathbb{R}^d$ be adapted with
$\mathbb{E}\big[\exp(\tfrac{1}{2}\int_0^T |\theta_t|^2\, dt)\big] < \infty$ (Novikov's
condition). Define
\[
...
assumptions, but the primary analysis in Theorem 3.1 uses synchronous coupling rather
than Girsanov.
\end{remark}
...
\mathbb{R}$ a bounded linear functional. Then there exists $\widetilde\varphi : E \to
\mathbb{R}$ with $\widetilde\varphi|_F = \varphi$ and
$\|\widetilde\varphi\|_{E^*} = \|\varphi\|_{F^*}$.
\end{theorem}

\begin{remark}[Application in Lemma 4.2]
The Hahn--Banach theorem is applied in Lemma 4.2 of the thesis to show that the
Fokker--Planck operator $\mathcal{L}^*$, initially defined on smooth test functions (a
dense subspace of $H^1$), extends to a bounded operator
$\mathcal{L}^* : H^1(\mathbb{R}^d) \to H^{-1}(\mathbb{R}^d)$ with the same operator
norm. This is essential for the Sobolev-space well-posedness of the SPDE.
\end{remark}

\subsection{Lumer--Phillips Theorem and Contraction Semigroups}

\begin{definition}[Dissipative Operator]
A linear operator $A : D(A) \subset H \to H$ on a Hilbert space $H$ is dissipative if
$\mathrm{Re}\langle Au, u \rangle \leq 0$ for all $u \in D(A)$.
\end{definition}

\begin{theorem}[Lumer--Phillips]
\label{thm:lumer-phillips}
Let $A$ be a closed densely defined operator on a Banach space $X$. Then $A$ generates
a $C_0$-contraction semigroup (i.e., $\|e^{tA}x\| \leq \|x\|$ for all $t \geq 0$,
$x \in X$) if and only if:
\begin{enumerate}[label=(\roman*)]
    \item $A$ is dissipative, and
    \item $(\mathrm{Id} - A)$ has dense range.
\end{enumerate}
\end{theorem}

\begin{remark}[Application to Fokker--Planck]
The Fokker--Planck operator $\mathcal{L}^*$ on $L^2(\mu^*)$ (where $\mu^*$ is the MFG
equilibrium measure) is dissipative:
$\langle \mathcal{L}^* \rho, \rho \rangle_{L^2(\mu^*)} = -\sigma^2 \|\nabla\rho\|_{L^2(\mu^*)}^2
\leq 0$ (assuming boundary terms vanish). By Lumer--Phillips, $\mathcal{L}^*$ generates
a contraction semigroup, and the spectral gap $\lambda_1 > 0$ (the smallest positive
eigenvalue) gives the exponential decay rate in Theorem 5.2.
\end{remark}

\subsection{Spectral Gap and Log-Sobolev Inequality}

\begin{definition}[Spectral Gap]
The spectral gap of the Fokker--Planck operator $\mathcal{L}^*$ on $L^2(\mu^*)$ is
\[
    \lambda_1 := \inf \left\{ \frac{-\langle \mathcal{L}^* f, f \rangle_{L^2(\mu^*)}}
    {\mathrm{Var}_{\mu^*}(f)} \;\middle|\; f \in D(\mathcal{L}^*), \;
    \mathrm{Var}_{\mu^*}(f) > 0 \right\}.
\]
\end{definition}

\begin{theorem}[Log-Sobolev Implies Spectral Gap]
\label{thm:log-sobolev}
If $\mu^*$ satisfies the log-Sobolev inequality with constant $C_{\mathrm{Lip}}$:
\[
    H(\mu \| \mu^*) \leq C_{\mathrm{Lip}}\, I(\mu \| \mu^*) \qquad \forall \mu \in
    \mathcal{P}_2(\mathbb{R}^d),
\]
where $H(\mu\|\mu^*) = \int \log\frac{d\mu}{d\mu^*}\, d\mu$ and
$I(\mu\|\mu^*) = \int |\nabla \log\frac{d\mu}{d\mu^*}|^2\, d\mu$, then
$\lambda_1 \geq 1/C_{\mathrm{Lip}}$.
\end{theorem}

\begin{proof}
The log-Sobolev inequality implies the Poincar\'e inequality
$\mathrm{Var}_{\mu^*}(f) \leq C_{\mathrm{Lip}} \int |\nabla f|^2\, d\mu^*$ (take
$\mu = f^2 \mu^*$, expand $H$ to second order). The Poincar\'e inequality is equivalent
to $\lambda_1 \geq 1/C_{\mathrm{Lip}}$.
\end{proof}

\begin{remark}[Log-Sobolev in financial applications]
The log-Sobolev inequality is a strong structural assumption; it typically requires the
equilibrium measure $\mu^*$ to satisfy Bakry--\'Emery criteria (e.g., strictly
log-concave density). \textbf{In financial markets, position distributions often
exhibit heavy tails (power laws), which violate the LSI and satisfy only the weaker
Poincar\'e inequality.} The LSI is used in Theorem 5.2 to establish an exponential
convergence rate as a theoretical upper bound; in practice, one expects algebraic rates
governed by the spectral gap. This limitation is acknowledged in the caveats of
Theorem 5.2. Recent work on Lata\l a--Oleszkiewicz inequalities and non-convex
isoperimetry (see Chapter 10) offers potential pathways to relax this assumption.
\end{remark}
...
\citet[Vol.\ I, Chapter 5]{carmonadelarue2018} for a comprehensive treatment. The HJB
equation
\[
    -\partial_t u = H^*(t,x,\mu,D_x u) + \frac{\sigma^2}{2}\Delta_x u, \qquad
    u(T,x,\mu) = g(x,\mu)
\]
is the PDE characterisation of the value function, and its unique solution (under
