...
\begin{enumerate}
    ...
    disequilibrium.
    \item \textbf{Formal verification in Lean 4}. We provide Lean 4 formalisations
    ...
    distance for Gaussian measures. As detailed in Appendix~B's provenance caveat,
    these proofs were written and reviewed for correctness but have not yet been
    compiled against a live Mathlib installation.
\end{enumerate}

\paragraph{Chapter organisation.} \Cref{sec:alpha-mfg-framework} reviews the
MFG framework and the LQ specification. \Cref{sec:alpha-signals} derives
...
are
\begin{align}
    b(t,x,\mu,\alpha) &= \kappa(\bar\mu - x) + \alpha, \notag\\
    f(t,x,\mu,\alpha) &= -\left( x^\top Q x + \frac{\lambda}{2}\|\alpha\|^2 \right), \notag\\
    g(x,\mu) &= -(x-\bar\mu_T)^\top P (x - \bar\mu_T),
...
\textbf{Correction note:} an earlier version of the $B_t$ equation above had
the sign of the $A_tB_t/\lambda$ cross term reversed; see Appendix~A for the
corrected derivation and the empirical check that confirmed it. Since this
chapter's terminal condition $B_T=-2P$ is, in magnitude, the same regime in
which the sign error has a large quantitative effect (unlike Chapter 7's
$B_T=0$ regime, where the effect is comparatively small), any specific
numerical values for the four alpha signals, the mean-reversion speed
$\kappa_t$, the factor exposure $\beta_t$, or the calibrated coefficient
$\eta_2$ reported elsewhere in this chapter that were computed prior to this
correction should be treated as stale and recomputed before being relied upon.
...
The most direct approach calibrates the LQ-MFG model parameters
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{p{1.8cm}p{2.1cm}p{2.6cm}p{2.0cm}p{2.0cm}}
\toprule
...
To ensure the correctness of key mathematical derivations, we have formalised
several properties using the Lean 4 interactive theorem prover. \textbf{As detailed
in the provenance caveat at the start of Appendix~B, these proofs were written and
reviewed for mathematical correctness but were not compiled against a live
Lean~4/Mathlib installation while writing this thesis; they should not be treated
as machine-verified until built successfully via \texttt{lake build}.} The
