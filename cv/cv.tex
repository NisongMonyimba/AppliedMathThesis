...
% Section heading: bold, with a thin rule underneath
\titleformat{\section}
  {\large\bfseries}
  {}{0em}{}
  [{\vspace{1pt}\titlerule}]
...
% Entry header: bold left title, right-aligned date on the same line
% (uses a stretchy minimum gap, not bare \hfill, so long titles never
% collide with the date)
\newcommand{\cventry}[2]{%
  \par\noindent\textbf{#1}\hspace{6pt plus 1fil}#2\par\vspace{1pt}
}
% Italic subline (scholarships, honors, etc.)
\newcommand{\cvsub}[1]{%
...
\cventry{Biomedical Engineering, B.Sc. --- Kwame Nkrumah University of Science \& Technology (KNUST)}{Graduated May 2021}
\cvsub{MasterCard Foundation Scholar; Provost's List; First Class Honours}
...
% ---------- Master's Applied Project & Capstone ----------
